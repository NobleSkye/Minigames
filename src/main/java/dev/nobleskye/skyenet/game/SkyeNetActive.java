...
public class SkyeNetActive {
    private final SkyeNetConfig config;

    public final GameSpace gameSpace;
    private final SkyeNetMap gameMap;

    private final Object2ObjectMap<PlayerRef, SkyeNetPlayer> participants;
    private final SkyeNetSpawnLogic spawnLogic;
    private final SkyeNetStageManager stageManager;
    private final boolean ignoreWinState;
    private final SkyeNetTimerBar timerBar;
    private final ServerWorld world;

    private SkyeNetActive(GameSpace gameSpace, ServerWorld world, SkyeNetMap map, GlobalWidgets widgets, SkyeNetConfig config, Set<PlayerRef> participants) {
        ...
        for (PlayerRef player : participants) {
            this.participants.put(player, new SkyeNetPlayer());
        }

        this.stageManager = new SkyeNetStageManager();
        this.ignoreWinState = this.participants.size() <= 1;
        this.timerBar = new SkyeNetTimerBar(widgets);
    }

    public static void open(GameSpace gameSpace, ServerWorld world, SkyeNetMap map, SkyeNetConfig config) {
        gameSpace.setActivity(game -> {
            Set<PlayerRef> participants = gameSpace.getPlayers().participants().stream()
                    .map(PlayerRef::of)
                    .collect(Collectors.toSet());
            GlobalWidgets widgets = GlobalWidgets.addTo(game);
            SkyeNetActive active = new SkyeNetActive(gameSpace, world, map, widgets, config, participants);
    ...
    private void tick() {
        ...
        SkyeNetStageManager.IdleTickResult result = this.stageManager.tick(time, gameSpace);
